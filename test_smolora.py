...
def test_lora_trainer_init_and_inference():
    with patch("smoLoRA.AutoModelForCausalLM") as mock_model_cls, \
         ...
         patch("smoLoRA.torch.device") as mock_device, \
         patch("smoLoRA.torch.backends.mps.is_available", return_value=False):
...
def test_lora_trainer_train_and_save(tmp_path):
    with patch("smoLoRA.AutoModelForCausalLM") as mock_model_cls, \
         ...
         patch("smoLoRA.torch.device") as mock_device, \
         patch("smoLoRA.torch.backends.mps.is_available", return_value=False), \
         patch("smoLoRA.torch.mps.empty_cache") as mock_empty_cache:
        from smoLoRA import SmoLoRA
...
def test_smolora_initialization_with_different_parameters():
...
def test_smolora_inference_with_custom_parameters():
...
def test_smolora_train_creates_checkpoint_directory():
    ...
    with patch("smoLoRA.AutoModelForCausalLM") as mock_model_cls, \
        ...
        mock_trainer_model.save_pretrained.assert_called_with("/custom/output/adapter_checkpoint")
        
        # Verify adapter_checkpoint attribute was set
        assert trainer.adapter_checkpoint == "/custom/output/adapter_checkpoint"

def test_smolora_dataset_mapping():
...
def test_smolora_error_handling():
...
def test_smolora_save_workflow():
    ...
    with patch("smoLoRA.AutoModelForCausalLM") as mock_model_cls, \
        ...
        assert mock_model_cls.from_pretrained.call_count >= 2  # Once for init, once for save
        ...
        mock_merged_model.save_pretrained.assert_called_with("/test/output/final_merged")
